      ...
      iter!=auxmap->end(); iter++)
  {
    ...
    */
         ...
         vit!=(*iter).second.end();vit++)
    {
      ...
      if (((*vit).type=="SensDet" or (*vit).type=="PD" ) and (*vit).value=="PhotonDetector")
      {
       ...
       /* G4VSensitiveDetector* mydet =SDman->FindSensitiveDetector((*vit).value);
        ...
        { */
          ...
          if(G4Threading::IsMasterThread()) fDetectIds.insert(std::pair<G4String,G4int>((*iter).first->GetName()+"_PV",sid++));
